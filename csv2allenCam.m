function [cam_vector, cam_target] = csv2allenCam(xml_fn)
% code provided by chongtianyifa
...
xml_data=csvread(xml_fn, 1, 1);
xml_data=xml_data(:,1:9);
slice_edge=zeros(size(xml_data,1),12);
cam_vector=zeros(size(xml_data,1),3); % each row for a camvector
cam_target=zeros(size(xml_data,1),3); % each row for a cametarget
for id_slice = 1:size(xml_data,1)
    o = xml_data(id_slice,1:3);
    u = xml_data(id_slice,4:6);
    v = xml_data(id_slice,7:9);
